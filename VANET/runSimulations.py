...
def doOneMeasure(portApp, input):
    ...
    #cmd = lambda x : f"./waf --run-no-build \"examples/NS3Scenario/vanet-test --numNodes=" + x["numNodes"] + " --procTime1=" + x["procTime1"] + " --nPackets1=" + x["nPackets1"] + " --mobTrace=" + x["seed"] + " --scenario=" + x["scenario"]+ "\""
...
def getInputsScenario1(scheme, numKeywords, numD, nLin):
    pT, nP = scheme.getAlgMeasures("enc", numKeywords, nbrLin=nLin)
    return {"nPackets1": str(ceil(nP / 1.024)), "procTime1": str(pT*1000)}
...
def main():
    ...
    numNodes = [58]
    numDisjunctions = [1, 2]
    #numKeywords = [1, 5, 10, 15, 30, 50, 100]
    numKeywords = [10]
    #numLines = [10]
    numLines = [75, 125, 150, 175]
    ...
    doneMeasures = readMeasures("measuresTest","test10")
    ...
    if len(lstRes) > 0:
        print("HI")
        writeMeasures(lstRes,"measuresTest","test10" )
main()
