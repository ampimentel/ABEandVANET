...
*/
int txSend = 0, rxReceive = 0, echoServerReceive = 0;
...
void setupMobility (int mobTrace, std::string mobFile, NodeContainer nodes, NodeContainer baseStations) {
  ...
  if(mobTrace >= 1 && mobTrace <= 10)
  {
    ...
    uint64_t mobility_seed = mobTrace;
    ObjectFactory pos;
...
void generateTraffic(WaveBsmHelper m_waveBsmHelper, NodeContainer nodes, float end, float start, int dataRate, std::vector <double> &m_txSafetyRangesSq) {
  ...
  for(uint32_t i = 0; i < num_nodes/10; i++) {
    generate_node_udp_traffic(nodes.Get(i+15), num_nodes, end, start + i*5, dataRate, 5, rand);
  }
...
void setReplayNodes(uint8_t srcPort, uint8_t destPort, NodeContainer replayNodes, int nPackets, Time pTime,  float end, float start, float node_delay=0){
  for(uint32_t i = 0; i < replayNodes.GetN(); i++) {
    Ptr<SendOnRecv> appRecv = CreateObject<SendOnRecv> ();
    appRecv->Setup (srcPort, destPort, 1024, nPackets, DataRate ("32Kbps"), pTime);
    appRecv->SetTxCallback(MakeCallback(&tracePacketSent));
    replayNodes.Get(i)->AddApplication (appRecv);
    appRecv->SetStartTime (Seconds (start + node_delay*i));
...
int main (int argc, char *argv[])
{
  ...
  cmd.AddValue ("traffic", "enable background traffic, 1 yes 0 no", traffic);
  cmd.Parse (argc, argv);
  ...
  setupMobility(mobTrace, "examples/NS3Scenario/highmob/mobility", nodes, baseStation);
  ...
  if(traffic){
 	...
 	generateTraffic(m_waveBsmHelper, nodes, end*0.8, start, dataRate, m_txSafetyRangesSq);
  ...
  if(scenario == 1){
      ...
      setBroadcastNodes(carListenPort, baseStation, end*0.8, start);
      setReplayNodes(carListenPort, 0, nodes, nPackets1, MilliSeconds(processingTime1), end*0.8, start, 0.75);
  }
  else if (scenario == 2) {
    uint8_t carListenPort = 47, bsListenPort = 48,  bsSendPort = 15;
    setBroadcastNodes(carListenPort, baseStation, end*0.8, start);
    setReplayNodes(carListenPort, bsListenPort, nodes, nPackets1, MilliSeconds(processingTime1), end*0.8, start, 0.75);
    setReplayNodes(bsListenPort, bsSendPort, baseStation, nPackets2, MilliSeconds(processingTime2), end*0.8, start);
    setEndNodes(bsSendPort, nodes, end*0.8, start);
  }
